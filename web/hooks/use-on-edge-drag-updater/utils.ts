...
export function getClosestHandle(
  ...
  for (const handle of handles) {
    const distance = Math.sqrt(
      Math.pow(handle.x - pos.x, 2) + Math.pow(handle.y - pos.y, 2),
    );
  ...
  return closestHandles.length === 1
    ...
    : // if multiple handles are layouted on top of each other we take the one with type = target because it's more likely that the user wants to connect to this one
      closestHandles.find((handle) => handle.type === "target") ||
        closestHandles[0];
}
...
export function getHandleLookup({
  ...
  for (const [, node] of nodeLookup) {
    if (node.internals.handleBounds) {
      ...
      const sourceHandles = getHandles(
        node,
        node.internals.handleBounds,
        "source",
        id,
      );
      const targetHandles = getHandles(
        node,
        node.internals.handleBounds,
        "target",
        id,
